...
use crate::{
    protocol::{self, Endpoint, Message},
    transport::Job,
};
use alloc::{string::String, vec::Vec};
use nonos_app_skeleton::{
    clipboard_paste, App, AppManifest, EventOutcome, InputEvent, InputKind, PaintBuffer,
    WindowKind, KEY_BACKSPACE, KEY_DOWN, KEY_ENTER, KEY_ESC, KEY_TAB, KEY_UP, MOD_CTRL, MOD_SHIFT,
};
use zeroize::Zeroize;
pub struct Chat {
    fields: [String; 4],
...
impl Chat {
    pub fn new() -> Self {
        Self {
            ...
            selected: false,
            messages: Vec::new(),
            status: "Enter your API URL, key and model.".into(),
            result: String::new(),
            job: None,
            models: false,
            scroll: 0,
    ...
    fn start(&mut self, models: bool) {
        ...
        let ep = match Endpoint::parse(&self.fields[0]) {
            Ok(e) => e,
            Err(e) => {
                self.status = e.into();
                return;
        ...
        if !models && self.fields[3].trim().is_empty() {
            self.status = "Please enter a message.".into();
            return;
        }
        ...
        if !models {
            rows.push(Message {
                role: "user",
                text: self.fields[3].clone(),
            });
        }
        match protocol::request(&ep, &self.fields[1], &self.fields[2], &rows, models) {
            Ok(r) => {
                self.job = Some(Job::new(ep, r));
                self.models = models;
                self.status = if models {
                    "Testing connection..."
                } else {
                    "Waiting for the model... (you can cancel at any time)"
                }
                .into();
            }
    ...
    fn edit(&mut self, text: &str) {
        ...
        if self.selected {
            self.fields[self.focus].zeroize();
            self.selected = false;
        }
        let limit = if self.focus == 3 {
            8000
        ...
        {
            if self.fields[self.focus].len() + c.len_utf8() > limit {
                break;
            }
        ...
        if self.focus < 3 {
            self.messages.clear();
            self.result.clear();
            self.status = "Settings changed. New chat started.".into();
        }
    }
    fn key(&mut self, e: InputEvent) {
        if e.flags & MOD_CTRL != 0 {
            match e.code {
                ...
                118 | 86 => {
                    let mut b = [0u8; 8192];
                    if let Ok(n) = clipboard_paste(&mut b) {
        ...
        match e.code {
            ...
            c => {
                if let Some(b) = crate::keymap::printable(c, e.flags & MOD_SHIFT != 0) {
                    self.edit(core::str::from_utf8(&[b]).unwrap_or(""));
                }
            }
        }
    }
...
impl App for Chat {
    fn manifest(&self) -> AppManifest {
        AppManifest {
            title: b"NONOS Chat",
            window_id: 0x43484154,
            kind: WindowKind::Normal,
            initial_x: 32,
            initial_y: 48,
            width: 1040,
            height: 660,
            input_kind_mask: 1 | 8 | 16 | 32,
        }
    }
    fn on_event(&mut self, e: InputEvent) -> EventOutcome {
        match e.kind {
            ...
            InputKind::ButtonDown => {
                let x = e.x;
                let y = e.y;
                if (82..126).contains(&y) {
                    self.focus = 0;
                    self.selected = false;
                } else if (156..200).contains(&y) {
                    self.focus = if x < (self.width as i32 * 3 / 5) {
                        1
                    } else {
                        2
                    };
            ...
            Ok(Some(r)) => {
                ...
                match protocol::answer(r, self.models) {
                    Ok(s) if self.models => {
                        self.result = s;
                        self.status =
                            "Connected. Choose a model ID from the list below and enter it above."
                                .into();
                        self.scroll = 0;
                    }
                    Ok(s) => {
                        self.messages.push(Message {
                            role: "user",
                            text: core::mem::take(&mut self.fields[3]),
                        });
                        self.messages.push(Message {
                            role: "assistant",
                            text: s,
    ...
    fn paint(&mut self, fb: &mut PaintBuffer) {
        ...
        fb.text_ttf(
            w.saturating_sub(240) as i32,
            24,
            "Private session chat",
            0xff9babbd,
            14.0,
        );
        fb.text_ttf(24, 58, "API URL", 0xffa8b5c6, 14.0);
        let kw = w * 3 / 5;
        fb.text_ttf(24, 132, "API key", 0xffa8b5c6, 14.0);
        fb.text_ttf(kw as i32 + 12, 132, "Model", 0xffa8b5c6, 14.0);
        field(
            fb,
            24,
            82,
        ...
        field(
            ...
            kw + 12,
            156,
            w.saturating_sub(kw + 36),
            44,
            &self.fields[2],
            self.focus == 2,
            self.selected && self.focus == 2,
        );
        button(fb, 24, 218, 180, "Test connection", false);
        button(fb, 220, 218, 175, "New chat", false);
        button(fb, 410, 218, 200, "Clear key", false);
        let top = 280;
        let bottom = h.saturating_sub(120);
        fb.fill_rect(
            24,
            top,
            w.saturating_sub(48),
        ...
        if !self.result.is_empty() {
            wrap(&self.result, chars, false, &mut lines);
        } else if self.messages.is_empty() {
            wrap(
                "Your AI chat in NONOS. Enter your API settings and send a message.",
                chars,
                false,
                &mut lines,
            );
            wrap(
                "Your key and chat history stay in memory for this session only.",
                chars,
                false,
                &mut lines,
            );
            ...
            for m in &self.messages {
                ...
                wrap(&m.text, chars, false, &mut lines);
        ...
        for (i, (label, line)) in lines.iter().skip(self.scroll).take(visible).enumerate() {
            fb.text_ttf(
                40,
                (top + 10) as i32 + (i * 23) as i32,
                line,
                if *label { 0xff63e2c6 } else { 0xffdde6f2 },
                16.0,
            );
        }
        ...
        field(
            ...
            24,
            y,
            w.saturating_sub(190),
            54,
            &self.fields[3].replace('\n', " / "),
            self.focus == 3,
            self.selected && self.focus == 3,
        );
        button(
            fb,
            w.saturating_sub(150),
            y + 6,
            126,
            if self.job.is_some() { "Cancel" } else { "Send" },
            true,
        );
...
fn button(fb: &mut PaintBuffer, x: u32, y: u32, w: u32, label: &str, accent: bool) {
    ...
    fb.text_ttf(
        ...
        y as i32 + 10,
        label,
        if accent { 0xff0d1320 } else { 0xffeef5ff },
        15.0,
    );
}
fn field(
    ...
    fb.fill_rect(
        ...
        w.saturating_sub(2),
        h.saturating_sub(2),
        if selected { 0xff294667 } else { 0xff182334 },
    );
...
fn wrap(text: &str, width: usize, label: bool, out: &mut Vec<(bool, String)>) {
    for paragraph in text.split('\n') {
        ...
        for c in paragraph.chars() {
            if count >= width {
                out.push((label, core::mem::take(&mut line)));
                count = 0;
            }
            line.push(c);
            count += 1;
